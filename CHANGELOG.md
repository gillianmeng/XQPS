...
### 修复（演示 / 部署）
- **根本原因**：`demo_users_fin_product.json`、`demo_users_fin_ops.json` 曾被列入 `.gitignore`，未进入仓库；部署环境只能回退到 `.example.json` 中的「示例员工/管理者」，与本地使用真实飞书拉取名单不一致。
- **处理**：与人力资源部、财富、研发、资管等 demo 一致，将上述两份 **真实员工 demo 配置纳入版本库**；`.gitignore` 中取消对这两文件的忽略；`.example.json` 仅保留空数组作为格式占位。
