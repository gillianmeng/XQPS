# 更新日志

## [v1.1.8] - 2026-03-27

### 内容（公告）
- `frozen_announcements.json`：**上级评分** 内置公告与后台一致更新为：「上级你好，请于「2026年07月04日 18:30」前提交上级评分！」（`2026年上半年` 与 `_default` 均已同步）。无 `admin_config` 或回退至快照时生效。

---

## [v1.1.7] - 2026-03-27

### 优化（上级评分）
- 下属评价区：**评分下拉**（工作业绩 / 通用能力 / 管理目标）与 **考核评语** 文本框默认蓝色描边与微发光，聚焦时略加强；禁用态仍保留提示感。仅作用于 `mgr_work_score` / `mgr_comp_score` / `mgr_lead_score` / `mgr_comment`，不影响名单上方筛选框。

### 优化（交互与滚动）
- **一级部门负责人调整 / 分管高管调整**：点击「查」后平滑滚动至「底线」提示下方；`components.html` 每次唯一 key，避免第二次点击不滚动；优先滚到自评详情面板标题。
- **上级评分**：「去评价」「查看」「去调整」调用 `jump_to_subordinate` 后同样滚动至名单下方评价区（锚点 + 面板标题）。

---

## [v1.1.6] - 2026-03-24

### 修复（演示 / 部署）
- **根本原因**：`demo_users_fin_product.json`、`demo_users_fin_ops.json` 曾被列入 `.gitignore`，未进入仓库；部署环境只能回退到 `.example.json` 中的「示例员工/管理者」，与本地使用真实飞书拉取名单不一致。
- **处理**：与人力资源部、财富、研发、资管等 demo 一致，将上述两份 **真实员工 demo 配置纳入版本库**；`.gitignore` 中取消对这两文件的忽略；`.example.json` 仅保留空数组作为格式占位。

---

## [v1.1.5] - 2026-03-24

### 修复（界面）
- 侧边栏「员工信息」：部门层级用 `normalize_dept_text` 过滤飞书占位（`-`、`—` 等），避免出现「一级部门 丨   丨   丨」；岗位为空时不单独显示多余分隔。

### 部署与演示（GitLab / 测试端口）
- 测试环境启动：`./run_demo.sh` 或 `PORT=<端口> ./run_demo.sh`（默认 8501），需 `APP_ENV=staging`、`ENABLE_DEMO_LOGIN=true`（脚本已设置）。
- **合并入口** `http://<主机>:<端口>/?demo_entry=1`（或 `demo_dept=all`）已合并 **六部门** demo（hr / wealth / rd / asset / fin_product / fin_ops）；单部门见脚本打印的 `demo_dept=` 链接。

---

## [v1.1.4] - 2026-03-24

### 修复（演示）
- **合并演示入口缺部门**：`demo_users_fin_product.example.json`、`demo_users_fin_ops.example.json` 此前为空的 `users`，在仅克隆仓库、未部署本地 `demo_users_fin_*.json` 时，合并入口无法出现金融产品与研究部、金融运营部账号。现已改为与 `demo_users_asset.example.json` 一致的占位示例（含部门与岗位说明）。
- `demo_users_asset.example.json`：下拉标签中增加「资产管理部」字样，便于在合并入口中识别。

---

## [v1.1.3] - 2026-03-24

### 新增（演示）
- 演示合并入口纳入 **金融产品与研究部**、**金融运营部**：`load_demo_users` 在 `demo_dept` 为空/`all` 时合并六套配置；单部门 `demo_dept=fin_product`、`demo_dept=fin_ops`。
- 仓库内提供 `demo_users_fin_product.example.json`、`demo_users_fin_ops.example.json`；真实 `demo_users_fin_product.json`、`demo_users_fin_ops.json` 列入 `.gitignore`（与其它部门 demo 一致）。

### 优化（界面与体验）
- 员工自评、上级评分、一级部门/分管高管「查」视图：**工作业绩 / 通用能力 / 管理目标** 文案与模块顺序与自评页一致（管理目标先于通用能力）。
- 上级评分侧栏与草稿提示用语对齐；「查」按钮样式统一。
- 飞书全表缓存、登录与主流程 **spinner** 自定义文案；Spinner 居中布局 CSS（`xqps_app/feishu_api.py`、`new_app.py`）。

### 脚本与文档
- `run_demo.sh`：支持环境变量 **`PORT`**（默认 8501），与 `bin/start_server.sh` 行为一致，打印的演示 URL 随端口变化。
- `bin/start_server.sh`、`get_open_ids.py`：补充金融产品与研究部、金融运营部相关说明。
- `README.md`、`docs/员工个人权限与设置.md`：演示入口由「四/三部门」更正为 **六部门** 合并逻辑与 URL。

### 文档（仓库整理，此前未单独发版）
- 合并 `docs/飞书字段与页面对应关系.md` 至 `docs/飞书表头与模块对应关系.md` 并删除旧文件；补充公告来源、后台角色等说明。
- 删除易过时文档与 `backup/` 历史脚本备份；根目录 `xqps.zip` 已忽略（见 `.gitignore`）。

---

## [v1.1.2] - 2025-03-19

### 修复
- 销售/非销售筛选逻辑修正：是否绩效关联奖金=否→销售，=是→非销售（财富顾问部等数据正确展示）

### 优化
- 部门绩效详情：去掉数据下钻、下载功能，表格仅作展示
- 部门绩效详情：带销售的部门，按部门展示总-销售-非销售三行（新增口径列）
- 截止日期公告：支持备用列名、从下属/全表记录兜底取值

---

## [v1.1.1] - 2025-03-19

### 修复
- 移除虚拟测试账号：资产管理部-分管高管（测试）、金融运营部-一级部门负责人（测试）
- 添加财富顾问部真实员工数据（demo_users_wealth.json）
- 清空 demo_users_wealth.example.json 占位用户（财富顾问部-管理者示例等）

---

## [v1.1.0] - 2025-03-19

### 新增
- 正式登录页支持扫码登录（二维码 + 浏览器链接）
- 侧边栏「📚 制度学习」模块，链接至雪球集团绩效管理制度（2025版）
- 演示入口合并三部门：`?demo_entry=1` 或 `?demo_entry=1&demo_dept=all` 显示人力资源部、研发质量保障部、财富顾问部全部人员

### 优化
- 上级评价：保存草稿需校验全部评分项完整；确认提交后下属表单只读、保存草稿禁用
- 上级评价：确认提交前校验全部下属已评分，未完成时提示名单
- 不在考核表内：温和提示「您不在本次考核期内」并退出
- 团队历史绩效「直属」按钮：蓝色样式、字体缩小

### 依赖
- 新增 `qrcode`、`pillow`（可选，未安装时仅显示链接登录）

### 文档
- README、docs/员工个人权限与设置.md 更新演示入口与合并逻辑说明
